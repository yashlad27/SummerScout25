...
const US_CITIES = ['New York', 'San Francisco', 'Seattle', 'Boston', 'Austin', 'Chicago', 'Los Angeles', 'Denver', 'Portland', 'Atlanta', 'Miami', 'Dallas', 'Houston', 'Phoenix', 'San Diego', 'San Jose', 'Philadelphia', 'Washington', 'Bay Area', 'Silicon Valley', 'Redmond', 'Mountain View', 'Palo Alto', 'Cupertino', 'Menlo Park', 'Santa Clara', 'Sunnyvale', 'Cambridge', 'Bellevue', 'Remote'];
...
function isUSLocation(location) {
    ...
    const hasStateCode = US_STATES.some(state => locationUpper.includes(state));
    
    // Check for common US cities
    const hasCityName = US_CITIES.some(city => location.includes(city));
    
    // Check for US indicators
    const hasUSIndicator = location.includes('USA') || 
                          location.includes('United States') || 
                          location.includes('U.S.');
    
    // Exclude international locations
    const internationalIndicators = ['Canada', 'UK', 'London', 'Europe', 'Asia', 'Singapore', 'India', 'China', 'Japan', 'Australia', 'Toronto', 'Montreal', 'Vancouver'];
    const isInternational = internationalIndicators.some(loc => location.includes(loc));
    
    return (hasStateCode || hasCityName || hasUSIndicator) && !isInternational;
}
...
function renderJobs() {
    ...
    // Apply US filter
    const usOnly = document.getElementById('usOnlyFilter').checked;
    if (usOnly) {
